...
public static class PiecePrefabManager
{
    ...
    private static GameObject CreateCategoryTab()
    {
        ...
        foreach (Text text in newTab.GetComponentsInChildren<Text>())
        {
    ...
    {
        yield return null;
        if(!Player.m_localPlayer.m_buildPieces) yield break;
        ...
        for (int i = 0; i < Hud.instance.m_pieceCategoryTabs.Length; ++i)
        {
            ...
            {
                Hud.instance.m_buildCategoryNames[i] = categoryName;
            }
        }
